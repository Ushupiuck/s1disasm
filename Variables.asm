...
f_scroll	= ramaddr ( $FFFFF745 )	; when, set game moves hscroll buffer to hscroll vram
v_scrYwrap	= ramaddr ( $FFFFF746 ) ; screen y-wrap value (2 bytes)
...
v_cameraPan	= ramaddr ( $FFFFF7AC )   ; Smooth Camera - how far the camera/view is panned to the left or right of Sonic (2 bytes)
v_cameraPanSpd	= ramaddr ( $FFFFF7AE )	; Smooth Camera - how fast it's panning to the left or right of Sonic (2 bytes)
...
v_lvllayoutfg	= ramaddr ( $FFFFF7F0 )	; level layout ROM address (4 bytes)
v_lvllayoutbg	= ramaddr ( $FFFFF7F4 )	; background layout ROM address (4 bytes)
...
v_screenposx_coarse	= ramaddr ( $FFFFFF24 )	; (Camera_X_pos - 128) / 256 (2 bytes)
